...
<div class="card card-style">
    <div class="content mb-0 mt-3">
        ...
        <form method="post" action="{{route('register.api')}}" >
            ...
            <div class="col-md-12 input-style-always-active has-borders no-icon mb-4" style="position: relative;margin-bottom: 15px !important;">
                <label for="upavasathi" class="color-highlight profess-tag">{{ __('messages.regform18') }} (Required)</label>
                <select required class="form-select upavasathi profess-tag-1" id="upavasathi" name="upavasathi" data-placeholder="{{ __('messages.regselect') }}"  style="border-color: rgba(0, 0, 0, 0.08) !important;">
                </select>
            </div>
    ...
    $(document).ready(function(){
        ...
        $('body').on('change', '.area', function () {
            $.ajax({
                ...
                {
                    $(".upavasathi").html('');
                    $(".upavasathi").append("<option label='Please Select' value=''>Select any one</option>");
                    $.each(data, function(i, item)
                    {
                        $(".upavasathi").append("<option value="+item.id+">"+item.name+"</option>");
                    });
                }
            });
        });
