...
<div class="ms-3 me-3 mb-5 alert alert-small rounded-s shadow-xl bg-red-dark s-alrt" role="alert">
 ...
 <div id="succmodal" class="modal fade s-alrt mt-4">
    <div class="modal-dialog modal-dialog-centered">
        ...
        <p class="boxed-text-l color-white opacity-70">Kindly try again</p>
            </div>
        </div>
    </div>
    </div>
...
<!--</div>-->
 ...
 <div id="succmodal" class="modal fade s-alrt mt-4">
    <div class="modal-dialog modal-dialog-centered">
        ...
        <h1 class="text-center mt-3 text-uppercase font-700 color-white">DHANYAVAD</h1>
        <p class="boxed-text-l color-white opacity-70">You'll be Contacted Shortly</p>
            </div>
        </div>
    </div>
    </div>
...
<div class="card card-style">
    <div class="content mb-0 mt-3">
        ...
        <form method="post" action="{{route('register.api')}}">
            ...
            <div class="input-style input-style-always-active has-borders no-icon validate-field mb-4">
                <input type="text" class="form-control validate-text textbox-n readonly" placeholder="{{ __('messages.regform4') }}" id="date"  name="yod" value="" max="{{date('Y')}}" required>
                <label for="form8" class="color-highlight">{{ __('messages.regform4') }} (Required)</label>
            </div>

            <div class="input-style input-style-always-active has-borders no-icon validate-field mb-4">
                <input type="text" class="form-control validate-text textbox-n readonly" id="date2"  placeholder="{{ __('messages.regform5') }}" name="doy" value="" max="{{date('Y')}}" required>
            ...
            <div class="respsubsub">
                <div class="input-style input-style-always-active has-borders no-icon validate-field mb-4">
                    <input type="text" class="form-control validate-text resposubsub" id="resposubsub" placeholder="{{ __('messages.regformother') }}" name="resposubsub" >
                    <label for="form3" class="color-highlight">{{ __('messages.regformother') }} </label>
                    <i class="fa fa-times disabled invalid color-red-dark"></i>
                    <i class="fa fa-check disabled valid color-green-dark"></i>
                </div>
            </div>
            ...
            <div class="input-style input-style-always-active has-borders no-icon validate-field mb-4">
                <input type="text" class="form-control validate-text" id="search" placeholder="{{ __('messages.regsearch') }}" name="mandala" required>
                <label for="search" class="color-highlight profess-tag">{{ __('messages.regform12') }} (Required)</label>
                <input type="hidden" id="searchhiddenid" name="area">
            </div>

            <!-- List items -->
            <div class="overflow-scroll searchlist mb-4"> 
                <ul class="list-group mt-3 searchlistul"> 
                    <div class="overflow-auto"></div>  
                </ul> 
            </div>

            <div class="col-md-12 input-style-always-active has-borders no-icon mb-4" style="position: relative;margin-bottom: 15px !important;">
                <label for="taluk" class="color-highlight profess-tag">{{ __('messages.regform13') }} </label>
            ...
            <div class="col-md-12 input-style-always-active has-borders no-icon mb-4" style="position: relative;margin-bottom: 15px !important;">
                <label for="district" class="color-highlight profess-tag">{{ __('messages.regform14') }} </label>
            ...
            <div class="col-md-12 input-style-always-active has-borders no-icon mb-4" style="position: relative;margin-bottom: 15px !important;">
                <label for="city" class="color-highlight profess-tag">{{ __('messages.regform15') }} </label>
    ...
    $(document).ready(function(){
        ...
        $.ajax({
            ...
            { 
                ...
                {
                    $("#single-select-field").append("<option selected disabled value='' >Select Mandala / Vasathi</option>");
                    $.each(data, function(i, item)
                    {
                        $("#single-select-field").append("<option value="+item.id+">"+item.name+" - "+item.name_kn+"</option>");      
                    });
                }
            }
        }); 
        ...
        $.ajax({
            ...
            { 
                ...
                {
                    $(".prof").append("<option selected disabled value='' >Select Profession</option>");
                    $.each(data, function(i, item)
                    {
        ...
        $.ajax({
            ...
            { 
                ...
                {
                    $(".resp").append("<option selected disabled value=''>Select Responsibilities</option>");
                    $.each(data, function(i, item)
                    {
            ...
            else
            {
                ...
                $.ajax({
                    ...
                    { 
                        ...
                        $(".resposub").append("<option selected disabled value=''>Choose any one option</option>");
                        $.each(data, function(i, item)
                        {
        ...
        $('.prof').on('change', function(){
                ...
                $.ajax({
                    ...
                    { 
                        ...
                        {
                            $(".are").html('');
                            $(".are").append("<option selected disabled value=''>Choose any one option</option>");
                            $.each(data, function(i, item)
                            {
