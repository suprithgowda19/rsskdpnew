      ...
      <div class="col-sm-12">
        <div class="card">
          ...
          <div class="card-body add-post">
            <form class="row needs-validation " novalidate="" method="post" action="{{route('admin.news.update', ["id"=>$news->news_id])}}" enctype='multipart/form-data'>
                @csrf
                @method('PUT')
              <div class="col-sm-12">
                <div class="mb-3">
                  <label for="validationCustom01">Title</label>
                  <input class="form-control" id="validationCustom01" type="text" name="title" placeholder="Add News Title" value="{{$news->title}}" required>
                </div>
                <div class="mb-3">
                  <label for="validationCustom02">About</label> <span>(Only 25 words)</span>
                  <input class="form-control" id="validationCustom02" type="text" name="about" placeholder="small description" value="{{$news->about}}" required="">
                </div>
                <div class="mb-3">
                  <label class="form-label" for="validationTextarea">Content</label>
                  <textarea id="editor" class="form-control is-invalid"  name="textbox" cols="10" rows="2" required="">{{$news->descp}}</textarea>
                </div>
                <div class=" mb-3">
                ...
                <div class="mb-3">
                    <figure class="col-md-3 col-6 img-hover hover-1" itemprop="associatedMedia" itemscope="">
                        <a href="{{asset('uploads/news')}}/<?=$news->image;?>" itemprop="contentUrl" data-size="1600x950" target="_blank">
                          <div>
                              <img src="{{asset('uploads/news')}}/<?=$news->image;?>" itemprop="thumbnail" alt="Image description" class="img-thumbnail" width="150" height="90">
                              </div>
                        </a>
                        <figcaption itemprop="caption description"><?=$news->image;?></figcaption>
                    </figure>
